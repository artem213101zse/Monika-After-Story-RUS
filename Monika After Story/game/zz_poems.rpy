...
init 5 python:
    addEvent(
        Event(
            ...
            prompt="Можно мне еще раз прочитать одно из твоих стихотворений?",
            category=["литература"],
            pool=True,
            unlocked=True,
...
label monika_showpoem:
    ...
    python:
        ...
        poems_list = [
            ("Дыра в стене (Part 1)", poem_m1, False, False),
            ("Дыра в стене (Part 2)", poem_m21, False, False),
            ("Сохрани меня", poem_m2, False, False),
    ...
    if not is_sad:
        m 3eka "Я надеюсь, тебе понравилось, [player]."

    m 1eka "Хочешь прочитать еще какое-нибудь стихотворение?{nw}"
    $ _history_list.pop()
    menu:
        m "Хочешь прочитать еще какое-нибудь стихотворение?{fast}"
